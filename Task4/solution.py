...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            p_distribution, _ = self.pi(state)
            action = p_distribution.sample()
            _, logp = self.pi(state, action)
            value = self.v(state)
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        Values_next = np.delete(vals,0)
        Values = np.delete(vals,len(vals) - 1)
        deltas = rews[:-1] + self.gamma*Values_next - Values

        self.discount_factor = self.gamma * self.lam
        self.phi_buf[path_slice] = discount_cumsum(deltas, self.discount_factor)
        #self.phi_buf[path_slice] = discount_cumsum(deltas, discount_factor)
        ...
        rews_to_go = np.zeros(rews.shape[0])
        #print(rews)
        for i in reversed(range(len(rews))):
            rews_to_go[i] = rews[i]
            if i + 1 < len(rews):
                rews_to_go[i] += rews_to_go[i + 1]
            rews_to_go[i] = rews_to_go[i] * self.gamma**i
        self.ret_buf[path_slice] = rews_to_go[:len(rews)-1]
...
class Agent:
    ...
    def v_update(self, data):
        ...
        for _ in range(100):
            ...
            mse = nn.MSELoss()
            value_function_loss = mse(value, ret)
    ...
    def get_action(self, obs):
        ...
        pi, _ = self.ac.pi(torch.tensor(np.array([obs])).float())

        #action = pi.sample()[0]

        action_multi_dim = pi.sample()
        action = action_multi_dim.squeeze()
        action = action.item()
