        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    torch.autograd.set_detect_anomaly(True) #modified
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.prior = MultivariateDiagonalGaussian(
            ...
            -2.3*torch.ones(in_features * out_features)
        )
        ...
        rho = torch.nn.Parameter(-2.3*torch.ones((in_features * out_features)))
        ...
        if self.use_bias:
            ...
            rho_bias = torch.nn.Parameter(-5*torch.ones(out_features)) #modified
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.use_bias:
            ...
            blprior = torch.sum(self.prior.log_likelihood(bias)) # bias likelihood
        else:
