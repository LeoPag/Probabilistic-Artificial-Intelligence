...
class Model(object):
    ...
    def predict(self, x: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        for i in range(0,len(gp_mean)):
            if(gp_mean[i] + 3*gp_std[i] > 35.5) and (gp_mean[i] < 35.5):
                predictions[i] = 35.55

        for i in range(0,len(gp_mean)):
            if(gp_mean[i] - 2*gp_std[i] > 35.5):
                predictions[i] = gp_mean[i] - 2*gp_std[i]
    ...
    def fit_model(self, train_x: np.ndarray, train_y: np.ndarray):
        ...
        model = GaussianProcessRegressor(kernel=k, optimizer='fmin_l_bfgs_b', n_restarts_optimizer=4)
